...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "doctor",
		Short: "Check CLI health (composite: brain dir + sqlite-vec deps + automations + schedule + Python deps)",
		Example: `  local-brain-pp-cli doctor
  local-brain-pp-cli doctor --json --full
  local-brain-pp-cli doctor --fail-on error`,
		RunE: func(cmd *cobra.Command, args []string) error {
			report := map[string]any{}
			// Brain-specific section: this is the differentiating "killer command"
			// — every fact an operator otherwise gathers from 7 separate probes.
			br := brainDoctor(cmd.Context(), fullReport)
			report["brain"] = br
			report["ok"] = br.OK
	...
	cmd.Flags().BoolVar(&fullReport, "full", false, "Include per-namespace stats (slower)")
	return cmd
}
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("local-brain-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'local-brain-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'local-brain-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'local-brain-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'local-brain-pp-cli sync' to refresh."
	}
	return report
}
