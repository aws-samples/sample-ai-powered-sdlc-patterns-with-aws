...
Exit codes & warnings:
  ...
  local-brain-pp-cli sync

  # Sync specific resources only
  local-brain-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  local-brain-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  local-brain-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  local-brain-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  local-brain-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "custom_automation":
		return db.UpsertCustomAutomation(data)
	case "embeddings":
		return db.UpsertEmbeddings(data)
	case "index":
		return db.UpsertIndex(data)
	case "memory":
		return db.UpsertMemory(data)
	case "memory_automation":
		return db.UpsertMemoryAutomation(data)
	case "namespace":
		return db.UpsertNamespace(data)
	case "schedule":
