...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  local-brain-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  local-brain-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"customautomation", "embeddings", "index", "memory", "memory-bisect", "memory-dedupe", "memory-patterns", "memory-watch", "memoryautomation", "namespace", "namespace-stats", "schedule", "schedule-next-runs",  }
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  local-brain-pp-cli workflow status

  # Show status as JSON
  local-brain-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("local-brain-pp-cli")
			}
