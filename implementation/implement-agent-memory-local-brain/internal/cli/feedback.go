...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("LOCAL_BRAIN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("LOCAL_BRAIN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "local-brain-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local-brain-pp-cli/feedback.jsonl.
When ` + "`LOCAL_BRAIN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`LOCAL_BRAIN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  local-brain-pp-cli feedback list
  local-brain-pp-cli feedback list --limit 5
  local-brain-pp-cli feedback list --json`,
