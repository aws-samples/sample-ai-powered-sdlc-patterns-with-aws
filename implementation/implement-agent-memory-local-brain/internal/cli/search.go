...
In local mode: searches locally synced data only.`,
  ...
  local-brain-pp-cli search "error timeout"

  # Force local search only
  local-brain-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  local-brain-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  local-brain-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /memory/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get("/memory/search", map[string]string{
					"query": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'local-brain-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "memory":
				results, err = db.SearchMemory(query, limit)
			case "":
				// Search all FTS-enabled tables individually to avoid duplicates.
				seen := make(map[string]bool)
				_ = seen // prevent unused error when no FTS tables exist
				{
					partial, searchErr := db.SearchMemory(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search memory failed: %w", searchErr)
					}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/local-brain-pp-cli/data.db)")
