...
func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "http://localhost/local-brain",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("LOCAL_BRAIN_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "local-brain-pp-cli", "config.json")
	}
	cfg.Path = path
	...
	if v := os.Getenv("LOCAL_BRAIN_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
